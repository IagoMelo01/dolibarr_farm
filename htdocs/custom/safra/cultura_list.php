...
/**
 *   	\file       cultura_list.php
 *		\ingroup    safra
 *		\brief      List page for cultura
 */
...
require_once __DIR__.'/class/cultura.class.php';
...
$hookmanager->initHooks(array('culturalist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('safra', 'cultura', 'write');
	$permissiontodelete = $user->hasRight('safra', 'cultura', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
//$help_url = "EN:Module_Cultura|FR:Module_Cultura_FR|ES:Módulo_Cultura";
$help_url = '';
$title = $langs->trans("Culturas");
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/safra/cultura_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/safra/cultura_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendCulturaRef";
$modelmail = "cultura";
$objecttmp = new Cultura($db);
