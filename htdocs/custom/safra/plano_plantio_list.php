...
/**
 *   	\file       plano_plantio_list.php
 *		\ingroup    safra
 *		\brief      List page for plano_plantio
 */
...
// load safra libraries
require_once __DIR__.'/class/plano_plantio.class.php';
...
$hookmanager->initHooks(array('plano_plantiolist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('safra', 'plano_plantio', 'write');
	$permissiontodelete = $user->hasRight('safra', 'plano_plantio', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Plano_Plantio';
	$objectlabel = 'Plano_Plantio';
...
//$help_url = "EN:Module_Plano_Plantio|FR:Module_Plano_Plantio_FR|ES:Módulo_Plano_Plantio";
$help_url = '';
$title = $langs->trans("Plano_Plantios");
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/safra/plano_plantio_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/safra/plano_plantio_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendPlano_PlantioRef";
$modelmail = "plano_plantio";
$objecttmp = new Plano_Plantio($db);
...
if (in_array('builddoc', $arrayofmassactions) && ($nbtotalofrecords === '' || $nbtotalofrecords)) {
	...
	print $formfile->showdocuments('massfilesarea_safra', '', $filedir, $urlsource, 0, $delallowed, '', 1, 1, 0, 48, 1, $param, $title, '', '', '', null, $hidegeneratedfilelistifempty);
}
