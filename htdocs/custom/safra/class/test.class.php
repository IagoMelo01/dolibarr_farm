...
class Test extends CommonObject
{
	 ...
	 */
	public $table_element = 'safra_test';
	...
	/**
	 * @var string String with name of icon for test. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'test@safra' if picto is file 'img/object_test.png'.
	 */
	...
	//  */
	// public $table_element_line = 'safra_testline';
	...
	//  */
	// protected $childtablesoncascade = array('safra_testdet');
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->safra->test->read) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && !empty($user->rights->safra->test->test_advance->validate))))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$dirsource = $conf->safra->dir_output.'/test/'.$oldref;
				$dirdest = $conf->safra->dir_output.'/test/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->safra->dir_output.'/test/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/safra/test_card.php', 1).'?id='.$this->id;
	...
	*/
	...
	public function getNextNumRef()
	{
		...
		if (getDolGlobalString('SAFRA_TEST_ADDON')=='') {
			$conf->global->SAFRA_TEST_ADDON = 'mod_test_standard';
		}

		if (getDolGlobalString('SAFRA_TEST_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('SAFRA_TEST_ADDON').".php";
			$classname = getDolGlobalString('SAFRA_TEST_ADDON');
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			...
			} elseif (getDolGlobalString('TEST_ADDON_PDF')) {
				$modele = getDolGlobalString('TEST_ADDON_PDF');
			}
		}
