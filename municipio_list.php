...
/**
 *   	\file       municipio_list.php
 *		\ingroup    safra
 *		\brief      List page for municipio
 */
...
require_once __DIR__.'/class/municipio.class.php';
...
$hookmanager->initHooks(array('municipiolist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('safra', 'municipio', 'write');
	$permissiontodelete = $user->hasRight('safra', 'municipio', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Municipio';
	$objectlabel = 'Municipio';
...
//$help_url = "EN:Module_Municipio|FR:Module_Municipio_FR|ES:Módulo_Municipio";
$help_url = '';
$title = $langs->trans("Municipios");
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/safra/municipio_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/safra/municipio_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendMunicipioRef";
$modelmail = "municipio";
$objecttmp = new Municipio($db);
