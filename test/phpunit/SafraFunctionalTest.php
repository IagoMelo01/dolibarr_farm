...
/**
 * \file    test/phpunit/SafraFunctionalTest.php
 * \ingroup safra
 ...
 * @package Testsafra
 */
class SafraFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	private static $module_id = 500010; // TODO: autodetect?
	...
	{
		$this->url('/custom/safra/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('safra/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/safra/admin/about.php');
		$this->authenticate();
		return $this->assertContains('safra/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('safrawidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modSafra_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
