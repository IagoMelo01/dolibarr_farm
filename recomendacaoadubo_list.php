 ...
 *		\ingroup    safra
 *		\brief      List page for recomendacaoadubo
 */
...
$hookmanager->initHooks(array('recomendacaoadubolist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->hasRight('safra', 'recomendacaoadubo', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'RecomendacaoAdubo';
	$objectlabel = 'RecomendacaoAdubo';
...
//$help_url = "EN:Module_RecomendacaoAdubo|FR:Module_RecomendacaoAdubo_FR|ES:Módulo_RecomendacaoAdubo";
$help_url = '';
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/safra/recomendacaoadubo_card.php', 1).'?id='.$id);
	exit;
}
...
$topicmail = "SendRecomendacaoAduboRef";
$modelmail = "recomendacaoadubo";
$objecttmp = new RecomendacaoAdubo($db);
