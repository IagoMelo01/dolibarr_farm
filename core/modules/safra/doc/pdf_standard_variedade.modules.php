...
class pdf_standard_variedade extends ModelePDFVariedade
{
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		/*
		if (getDolGlobalString('MAIN_GENERATE_VARIEDADE_WITH_PICTURE'))
		{
		...
		*/
		...
		if ($conf->safra->dir_output.'/variedade') {
			$object->fetch_thirdparty();

			// Definition of $dir and $file
			if ($object->specimen) {
				$dir = $conf->safra->dir_output.'/variedade';
				$file = $dir."/SPECIMEN.pdf";
			} else {
				$objectref = dol_sanitizeFileName($object->ref);
				$dir = $conf->safra->dir_output.'/variedade/'.$objectref;
