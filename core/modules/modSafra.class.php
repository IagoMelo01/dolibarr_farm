...
class modSafra extends DolibarrModules
{
	...
	public function __construct($db)
	{
		...
		if (!isset($conf->safra) || !isset($conf->safra->enabled)) {
		...
		$this->tabs = array();
		// Example:
		// $this->tabs[] = array('data'=>'objecttype:+tabname1:Title1:mylangfile@safra:$user->rights->safra->read:/safra/mynewtab1.php?id=__ID__');  					// To add a new tab identified by code tabname1
		// $this->tabs[] = array('data'=>'objecttype:+tabname2:SUBSTITUTION_Title2:mylangfile@safra:$user->rights->othermodule->read:/safra/mynewtab2.php?id=__ID__',  	// To add another new tab identified by code tabname2. Label will be result of calling all substitution functions on 'Title2' key.
		...
		$this->dictionaries=array(
			...
			'tabcond'=>array($conf->safra->enabled, $conf->safra->enabled, $conf->safra->enabled),
		...
		*/
		...
		$this->cronjobs = array(
			...
			//      'unitfrequency' => 3600,
			//      'status' => 0,
			//      'test' => '$conf->safra->enabled',
			//      'priority' => 50,
			//  ),
		);
		// Example: $this->cronjobs=array(
		//    0=>array('label'=>'My label', 'jobtype'=>'method', 'class'=>'/dir/class/file.class.php', 'objectname'=>'MyClass', 'method'=>'myMethod', 'parameters'=>'param1, param2', 'comment'=>'Comment', 'frequency'=>2, 'unitfrequency'=>3600, 'status'=>0, 'test'=>'$conf->safra->enabled', 'priority'=>50),
		//    1=>array('label'=>'My label', 'jobtype'=>'command', 'command'=>'', 'parameters'=>'param1, param2', 'comment'=>'Comment', 'frequency'=>1, 'unitfrequency'=>3600*24, 'status'=>0, 'test'=>'$conf->safra->enabled', 'priority'=>50)
		// );
		...
		$this->rights[$r][1] = 'Read objects of Safra'; // Permission label
		$this->rights[$r][4] = 'cultura';
		$this->rights[$r][5] = 'read'; // In php code, permission will be checked by test if ($user->rights->safra->cultura->read)
		$r++;
		$this->rights[$r][0] = $this->numero . sprintf("%02d", $r + 1); // Permission id (must not be already used)
		$this->rights[$r][1] = 'Create/Update objects of Safra'; // Permission label
		$this->rights[$r][4] = 'cultura';
		$this->rights[$r][5] = 'write'; // In php code, permission will be checked by test if ($user->rights->safra->cultura->write)
		$r++;
		$this->rights[$r][0] = $this->numero . sprintf("%02d", $r + 1); // Permission id (must not be already used)
		$this->rights[$r][1] = 'Delete objects of Safra'; // Permission label
		$this->rights[$r][4] = 'cultura';
		$this->rights[$r][5] = 'delete'; // In php code, permission will be checked by test if ($user->rights->safra->cultura->delete)
		$r++;
		...
		/* BEGIN MODULEBUILDER LEFTMENU CULTURA
		$this->menu[$r++]=array(
			'fk_menu'=>'fk_mainmenu=safra',      // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
		...
		$this->menu[$r++]=array(
			'fk_menu'=>'fk_mainmenu=safra,fk_leftmenu=cultura',	    // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
			'type'=>'left',			                // This is a Left menu entry
			'titre'=>'List_Cultura',
			'mainmenu'=>'safra',
		...
		$this->menu[$r++]=array(
			...
			'langs'=>'safra@safra',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'position'=>1000+$r,
			'enabled'=>'isModEnabled("safra")', // Define condition to show or hide menu entry. Use 'isModEnabled("safra")' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
		...
		*/
		...
		$this->menu[$r++]=array(
            ...
            'langs'=>'safra@safra',
            'position'=>1100+$r,
            // Define condition to show or hide menu entry. Use '$conf->safra->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
            'enabled'=>'$conf->safra->enabled',
            // Use 'perms'=>'$user->rights->safra->level1->level2' if you want your menu with a permission rules
            'perms'=>'1',
            'target'=>'',
		...
		$this->menu[$r++]=array(
            // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
            'fk_menu'=>'fk_mainmenu=safra,fk_leftmenu=safra_talhao',
            // This is a Left menu entry
            'type'=>'left',
            'titre'=>'Novo Talhâo',
            'mainmenu'=>'safra',
        ...
        $this->menu[$r++]=array(
            ...
            // This is a Left menu entry
            'type'=>'left',
            'titre'=>'Lista de Culturas',
            'mainmenu'=>'safra',
            'leftmenu'=>'safra_cultura',
            'url'=>'/safra/cultura_list.php',
            // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
            'langs'=>'safra@safra',
            'position'=>1100+$r,
        ...
        $this->menu[$r++]=array(
            // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
            'fk_menu'=>'fk_mainmenu=safra,fk_leftmenu=safra_cultura',
            // This is a Left menu entry
            'type'=>'left',
            'titre'=>'Nova Cultura',
            'mainmenu'=>'safra',
		...
		$this->menu[$r++]=array(
            ...
            // This is a Left menu entry
            'type'=>'left',
            'titre'=>'Lista de Variedades',
            'mainmenu'=>'safra',
            'leftmenu'=>'safra_variedade',
        ...
        $this->menu[$r++]=array(
            // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
            'fk_menu'=>'fk_mainmenu=safra,fk_leftmenu=safra_variedade',
            // This is a Left menu entry
            'type'=>'left',
            'titre'=>'Nova Variedade',
            'mainmenu'=>'safra',
            'leftmenu'=>'safra_variedade',
		...
		/* END MODULEBUILDER LEFTMENU CULTURA */
		// Exports profiles provided by this module
		$r = 1;
		/* BEGIN MODULEBUILDER EXPORT CULTURA */
		/*
		...
		$r++; */
		/* END MODULEBUILDER EXPORT CULTURA */

		// Imports profiles provided by this module
		$r = 1;
		/* BEGIN MODULEBUILDER IMPORT CULTURA */
		/*
		$langs->load("safra@safra");
		...
		$this->import_convertvalue_array[$r] = array(
			't.ref' => array(
				'rule'=>'getrefifauto',
				'class'=>(getDolGlobalString('SAFRA_CULTURA_ADDON')=='') ? 'mod_cultura_standard' : getDolGlobalString('SAFRA_CULTURA_ADDON')),
				'path'=>"/core/modules/commande/".(getDolGlobalString('SAFRA_CULTURA_ADDON')=='') ? 'mod_cultura_standard' : getDolGlobalString('SAFRA_CULTURA_ADDON')).'.php'
				'classobject'=>'Cultura',
		...
		$r++; */
		/* END MODULEBUILDER IMPORT CULTURA */
	}
	...
	public function init($options = '')
	{
		...
		//$result1=$extrafields->addExtraField('safra_myattr1', "New Attr 1 label", 'boolean', 1,  3, 'thirdparty',   0, 0, '', '', 1, '', 0, 0, '', '', 'safra@safra', '$conf->safra->enabled');
		//$result2=$extrafields->addExtraField('safra_myattr2', "New Attr 2 label", 'varchar', 1, 10, 'project',      0, 0, '', '', 1, '', 0, 0, '', '', 'safra@safra', '$conf->safra->enabled');
		//$result3=$extrafields->addExtraField('safra_myattr3', "New Attr 3 label", 'varchar', 1, 10, 'bank_account', 0, 0, '', '', 1, '', 0, 0, '', '', 'safra@safra', '$conf->safra->enabled');
		//$result4=$extrafields->addExtraField('safra_myattr4', "New Attr 4 label", 'select',  1,  3, 'thirdparty',   0, 1, '', array('options'=>array('code1'=>'Val1','code2'=>'Val2','code3'=>'Val3')), 1,'', 0, 0, '', '', 'safra@safra', '$conf->safra->enabled');
		//$result5=$extrafields->addExtraField('safra_myattr5', "New Attr 5 label", 'text',    1, 10, 'user',         0, 0, '', '', 1, '', 0, 0, '', '', 'safra@safra', '$conf->safra->enabled');
	...
	{
		return $this->_load_tables('/safra/sql/');
